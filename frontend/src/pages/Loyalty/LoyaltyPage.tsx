...
const LoyaltyPage = () => {
  ...
  // Check if the error is due to no active loyalty program
  if (err.response?.status === 404 && err.response?.data?.detail === "No active loyalty program") {
    setError("No active loyalty program found. Please create one to view analytics.");
    setAnalytics(null); // Clear analytics if no active program
  } else {
